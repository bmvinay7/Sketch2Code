...
function buildParts(prompt: string, imageBase64: string | undefined): Part[] {
...
export async function streamShapeCode(
  ...
  for (const modelName of MODEL_FALLBACK_CHAIN) {
    ...
    for (let attempt = 0; attempt <= MAX_RETRIES_PER_MODEL; attempt++) {
      ...
      try {
        ...
        lastError = error;
        if (emittedAny) {
          console.error(`[gemini] ${modelName} errored mid-stream:`, (error as Error)?.message ?? error);
          onChunk(commentForLanguage(language, "\nStream interrupted by Gemini. Output above may be incomplete."));
          return;
        }
        if (!isRetryableError(error) || attempt === MAX_RETRIES_PER_MODEL) break;
        const backoff = INITIAL_BACKOFF_MS * Math.pow(2, attempt);
        console.warn(`[gemini] ${modelName} stream attempt ${attempt + 1} failed, retrying in ${backoff}ms`);
        await sleep(backoff);
      }
    }
  ...
  if (isModelMissingError(error)) {
    onChunk(commentForLanguage(language, "No supported Gemini model is available for this API key."));
  } else if (/quota|free_tier|generate_content_free_tier/i.test(message)) {
    onChunk(commentForLanguage(language, "Gemini daily quota exceeded. Wait or upgrade the API plan."));
...
Problem context: ${problemContext ?? "None"}`;
  ...
  if (!result.ok) {
    ...
    const message = error?.message ?? "";
    if (isModelMissingError(error)) return "No supported Gemini model is available for this API key.";
    if (/quota|free_tier|generate_content_free_tier/i.test(message)) {
      return "Gemini daily quota exceeded. Wait or upgrade the API plan.";
    }
    ...
    if (error?.status === 429) return "Rate limit reached. Please wait a moment and try again.";
